...
4. edit `$HOME/locals/share/icons/breeze/index.theme` and make sure to add the following line to the `[Icon Theme]` section: `FollowsColorScheme=true` &ndash; this ensures that the icon theme properly adapts to the color scheme you set.
5. remove `$HOME/.cache/icon-cache.kcache` to be on the safe side; this removes all potentially cached and now stale Breeze icons from the cache, so the updated icons will be used instead (and cached now).
6. as with any local Kdenlive install outside the system directories, make sure that `XDG_DATA_DIRS` correctly points to your local stuff; that is, to `$HOME/local/share`. Failing to do so will Kdenlive not let see and use the updated Breeze icon set.
7. make sure that you've selected the Breeze icon set (and not Breeze dark or some other icon set).
